...
/**
 * Setup pagination for courses pagination template
 *
 * @param array $coursess An array of TalentLMS courses
 * @param int $page The current page
 * @return array An array of courses, along with the number of pages and the current page
 * */
function tl_setup_courses_pagination($courses, $page) {
	$currentpage = ($page) ? trim($page) : 0;

	$perpage = get_option('tl-courses-page-pagination-template-courses-per-page');
...
/**
 * Setup pagination for groups template
 *
 * @param array $groups An array of TalentLMS groups
 * @param int $page The current page
 * @return array An array of groups, along with the number of pages and the current page
 * */
function tl_setup_groups_pagination($groups, $page) {
	$currentpage = ($page) ? trim($page) : 0;

	$perpage = get_option('tl-groups-page-template-groups-per-page');

	$num_of_groups = 0;
	foreach ($groups as $group) {
		$num_of_groups++;
	}
	$numofpages = ceil($num_of_groups / $perpage);
	...
	return array('groups' => $groups, 'numofpages' => $numofpages, 'currentpage' => $currentpage);
}
...
function tl_sync_wp_tl_users($tl_users_to_wp, $wp_users_to_tl, $hard_sync, $all_users) {
	...
	foreach ($wp_users_to_tl as $user) {
		try {
			$new_user = TalentLMS_User::signup(array('first_name' => $user['first_name'], 'last_name' => $user['last_name'], 'email' => $user['email'], 'login' => $user['login'], 'password' => $user['login']));
		} catch (Exception $e) {
